...
await run('npm', ['run', 'test:action'], 120000);
await run('npm', ['run', 'test:animation:unit'], 90000);
await run('npm', ['run', 'test:mobile:unit'], 90000);
...
try {
  await ready();
  await run('npm', ['run', 'test:movement'], 180000);
  await run('npm', ['run', 'test:action:e2e'], 180000);
  await run('npm', ['run', 'test:visual'], 240000);
} finally {
