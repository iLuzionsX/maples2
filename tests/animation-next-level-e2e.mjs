...
const result = await page.evaluate(() => {
  ...
  g.player.beginDodge(dodgeVector);
  ...
  const visualQuaternionBeforeHit = enemy.assetVisual.quaternion.clone();
  const hpBefore = enemy.hp;
  enemy.takeHit(1, g.player.position.clone(), true);
  for (let i = 0; i < 5; i++) enemy.update(dt, g.player);
  const assetRootAngleDrift = visualQuaternionBeforeHit.angleTo(enemy.assetVisual.quaternion);
...
if (result.playerStrikeAccents < 3 || result.attackAnticipations < 3) errors.push('All three player combo event accents did not execute');
...
if (result.landingEvents < 1 || result.locomotionImpulseFrames < 1) errors.push('Dodge landing / locomotion impulse layer did not execute');
...
if (result.assetRootAngleDrift > 1e-5) errors.push(`Enemy flinch leaked onto imported model root: ${result.assetRootAngleDrift}`);
